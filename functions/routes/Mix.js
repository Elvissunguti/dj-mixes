...
router.post(
  ...
  async (req, res) => {
    ...
    const { title, thumbnailUrl, trackUrl } = req.body;
    const favouritedBy = [];

    if (!title || !thumbnailUrl || !trackUrl) {
      return res.status(400).json({ error: "Unable to create mix, missing required fields" });
    }
    ...
    const MixDetails = {
      title,
      thumbnail: thumbnailUrl,  // Using the URL from Firebase
      track: trackUrl,           // Using the URL from Firebase
      artist,
      userId: req.user._id,
      favouritedBy
    };
